...
const EnhancedSocialSharing: React.FC<EnhancedSocialSharingProps> = ({ 
  ...
  const handleCopyAll = async () => {
    try {
      ...
      showToast('تم نسخ جميع المحتويات!', 'success');
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">مشاركة محسنة</h2>
          <p className="text-gray-400">إدارة شاملة للمشاركة مع تحليلات متقدمة</p>
        </div>
        {onClose && (
      ...
      <div className="flex space-x-1 bg-gray-800 p-1 rounded-lg">
        {[
          { id: 'create', label: 'إنشاء المحتوى', icon: FiEdit },
          { id: 'schedule', label: 'جدولة المشاركة', icon: FiCalendar },
          { id: 'analytics', label: 'التحليلات', icon: FiBarChart }
        ].map(tab => (
          <button
            key={tab.id}
              ...
              <CardContent>
                <div className="space-y-6">
                  {selectedPlatforms.map(platformId => {
                    const content = generatedContent[platformId];
                    const platform = getPlatformInfo(platformId);
                    const isPlatformEditing = isEditing[platformId];
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="p-4 bg-gray-800 rounded-lg">
                  <h4 className="text-white font-medium mb-2">المنصات المحددة</h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedPlatforms.map(platformId => {
                      const platform = getPlatformInfo(platformId);
                      return (
                        <span key={platformId} className="px-3 py-1 bg-prepilot-purple-500/20 text-prepilot-purple-300 rounded-full text-sm">
      ...
      {/* Analytics Tab */}
      {activeTab === 'analytics' && (
        <SocialAnalyticsComponent reportId={report.id} />
      )}
