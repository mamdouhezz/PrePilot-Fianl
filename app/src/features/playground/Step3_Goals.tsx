...
import PlatformAffinityIndicator from '../../components/playground/PlatformAffinityIndicator';
        ...
        <SectionCard title="الخطوة 3: الأهداف والمنصات">
            <div className="space-y-8">
                ...
                <div className="pt-4 border-t border-gray-800">
                        ...
                        {PLATFORMS.map(platform => {
                            ...
                            return (
                             <button type="button" key={platform.id} onClick={() => handlePlatformToggle(platform.id)} className={`p-4 flex flex-col items-center justify-center text-center rounded-lg border-2 transition-all duration-200 relative ${isSelected ? 'border-prepilot-purple-500 bg-prepilot-purple-900/40' : `border-gray-700 bg-gray-800 hover:border-gray-500 ${isSuggested && !isDiscouraged ? 'shadow-lg shadow-prepilot-purple-900/50' : ''}`}`}>
                                 {isSuggested && !isDiscouraged && <span className="absolute top-1 left-1 text-[10px] bg-prepilot-purple-500 text-white px-1.5 py-0.5 rounded-full font-bold">مقترح</span>}
                                 {isDiscouraged && (
                                 ...
                                 {/* Platform Affinity Indicator */}
                                 <PlatformAffinityIndicator 
                    ...
                    <RealtimeTip fieldName="platforms" tips={realtimeTips} isLoading={areTipsLoading} />
                </div>
                ...
                <div className="pt-4 border-t border-gray-800">
                    ...
                    <RealtimeTip fieldName="seasons" tips={realtimeTips} isLoading={areTipsLoading} />
                </div>
            </div>
        </SectionCard>
    );
