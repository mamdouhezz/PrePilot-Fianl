...
import type { CampaignData, BrandContext, UIWarning, UserPreset } from '../../types';
import { predictInitialSettings } from '../engine';
import type { ContextualQuestion } from '../types/index';
import {
...
import { enhanceBrandDetails, preflightValidation, generateRealtimeValidationTips, generateBrandBriefFromText } from '../engine';
...
export function usePlaygroundForm(initialData: CampaignData | null, onSubmit: (form: CampaignData) => Promise<void>) {
    ...
    const [dynamicQuestions, setDynamicQuestions] = useState<ContextualQuestion[]>([]);
        ...
        const handler = setTimeout(() => {
            if (formData.industry) {
                ...
                generateRealtimeValidationTips(formData)
                    .then(tips => {
                        console.log('💡 AI Tips Received:', tips);
    ...
    useEffect(() => {
        ...
        // If subIndustry is selected, let the prediction engine decide
        if (formData.subIndustry) return;
    ...
    useEffect(() => {
        ...
        // If subIndustry is selected, prefer predicted platforms from the engine
        if (formData.subIndustry) return;
        const suggested = getSuggestedPlatforms(formData.industry, formData.goals);
        setFormData(prev => ({ ...prev, platforms: suggested }));
    }, [formData.industry, formData.subIndustry, formData.goals]);
    ...
    /**
     * EFFECT: Triggers the prediction engine whenever the user finalizes their
     * industry and sub-industry selection to auto-populate downstream form fields
     * and reveal contextual questions.
     */
    useEffect(() => {
        const industry = formData.industry;
        const subIndustry = formData.subIndustry;
        ...
        setFormData(prev => ({ ...prev, ...prediction.settings } as CampaignData));
    ...
    const handleLocationToggle = useCallback((location: string) => {
        setFormData(prevFormData => {
            const currentLocations = prevFormData.targetAudience.locations;
            let newLocations: string[];
            if (location === 'كل المدن الرئيسية') {
                newLocations = ['كل المدن الرئيسية'];
            } else {
                const tempLocations = currentLocations.filter(l => l !== 'كل المدن الرئيسية');
    ...
    const handleInterestToggle = useCallback((interestId: string) => {
        setFormData(prevFormData => ({
            ...
            targetAudience: {
                ...prevFormData.targetAudience,
                interests: prevFormData.targetAudience.interests.includes(interestId)
                    ? prevFormData.targetAudience.interests.filter(i => i !== interestId)
                    : [...prevFormData.targetAudience.interests, interestId],
            },
        }));
    }, []);

    const handleBehaviorToggle = useCallback((behaviorId: string) => {
        setFormData(prevFormData => ({
            ...prevFormData,
            targetAudience: {
                ...prevFormData.targetAudience,
                behaviors: prevFormData.targetAudience.behaviors.includes(behaviorId)
                    ? prevFormData.targetAudience.behaviors.filter(b => b !== behaviorId)
                    : [...prevFormData.targetAudience.behaviors, behaviorId],
            },
        }));
    }, []);

    const handleAgeToggle = useCallback((age: string) => {
        setFormData(prevFormData => ({
            ...prevFormData,
            targetAudience: {
                ...prevFormData.targetAudience,
                age: prevFormData.targetAudience.age.includes(age)
                    ? prevFormData.targetAudience.age.filter(a => a !== age)
                    : [...prevFormData.targetAudience.age, age],
            },
        }));
    }, []);
    ...
    const handleSeasonToggle = useCallback((season: string) => {
        setFormData(prevFormData => ({
            ...prevFormData,
            seasons: prevFormData.seasons.includes(season)
                ? prevFormData.seasons.filter(s => s !== season)
                : [...prevFormData.seasons, season],
        }));
    }, []);
    ...
    const handlePlatformToggle = useCallback((platformId: string) => {
        setFormData(prevFormData => ({
            ...prevFormData,
            platforms: prevFormData.platforms.includes(platformId)
                ? prevFormData.platforms.filter(p => p !== platformId)
                : [...prevFormData.platforms, platformId],
        }));
    }, []);
    ...
    const handleSubGoalToggle = useCallback((subGoal: string) => {
        ...
        setFormData(prevFormData => {
            const currentForPrimary = prevFormData.subGoals[primaryGoal] || [] as string[];
            const nextForPrimary = currentForPrimary.includes(subGoal)
                ? currentForPrimary.filter(sg => sg !== subGoal)
                : [...currentForPrimary, subGoal];
            return {
                ...prevFormData,
                subGoals: {
                    ...prevFormData.subGoals,
                    [primaryGoal]: nextForPrimary,
                },
            };
        });
    }, [primaryGoal]);
    ...
    const handleSecondaryGoalToggle = useCallback((goal: string) => {
        setFormData(prevFormData => {
            const currentPrimary = prevFormData.goals[0] || '';
            const currentSecondary = prevFormData.goals.slice(1);
            const nextSecondary = currentSecondary.includes(goal)
                ? currentSecondary.filter(g => g !== goal)
                : [...currentSecondary, goal];
            const nextGoals = currentPrimary ? [currentPrimary, ...nextSecondary] : nextSecondary;
            return {
                ...prevFormData,
                goals: nextGoals,
            };
