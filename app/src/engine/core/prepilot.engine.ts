...
import { getAudiencePersonaSummary } from '../modules/affinity.engine';
...
export const runCampaign = async (formData: CampaignData): Promise<CampaignReport | { errors: string[] }> => {
  try {
    ...
    // Step 7: Generate Audience Persona Summary for AI Context
    const audiencePersonaSummary = getAudiencePersonaSummary(formData.targetAudience);

    // Step 8: AI Content Generation (now with audience context)
